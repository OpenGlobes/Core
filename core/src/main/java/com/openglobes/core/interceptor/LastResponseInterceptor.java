 ...
 * @since 1.0
 */
public class LastResponseInterceptor extends AbstractResponseInterceptor<Response> {

    private final ISharedContext ctx;

    public LastResponseInterceptor(ISharedContext context) {
        ctx = context;
    }

    @Override
    public InterceptOperation onResponse(Response response, IInterceptorChain stack) {
        try {
            ctx.getSessionCorrelator()
               .getSessionByOrderId(response.getOrderId())
               .respond(response);
        } catch (SessionException ex) {
            Loggers.getLogger(LastResponseInterceptor.class.getCanonicalName()).log(Level.SEVERE,
                                                                                    ex.toString(),
                                                                                    ex);
        }
